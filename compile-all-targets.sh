...
echo -e "${H1}Compilation of all targets for clima $version${EH}"
...
cp target/x86_64-pc-windows-gnu/release/clima.exe build/x86_64-pc-windows-gnu/

# build the linux version
echo -e "${H2}Compiling the linux version${EH}"
cargo build --release
strip target/release/clima
mkdir build/x86_64-linux/
cp target/release/clima build/x86_64-linux/
